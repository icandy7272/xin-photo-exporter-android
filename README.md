...
| **Python 3** | 全新的 Mac 上第一次运行会提示安装「命令行开发者工具」（约 1.9 GB），点「安装」即可，见步骤 1。**不需要安装任何额外的库。** |
| 硬盘空间 | 相册本身几百 MB 到几 GB。全新 Mac 还要一次性下载：命令行工具约 1.9 GB；若走免费模拟器再加约 6 GB（用 MuMu 则不需要这 6 GB）。 |
...
4. 如果**弹出一个对话框问你要不要安装「命令行开发者工具」**，点「安装」，等它装完
   （**约 1.9 GB，视网速可能要十几分钟**）。装完再执行一次 `python3 --version` 确认。

   > 这是 macOS 的正常行为：全新的 Mac 上 `python3` 只是个占位程序，你一运行它，
   > 系统就会主动提示安装。Python 3 是这套工具里的一个组件，装完你会得到 Python 3.9.6，
   > 本工具在这个版本上完整测试过。

5. 如果既没看到版本号、也没弹窗，手动运行这行触发安装：
   ```bash
...
> **也可以跳过这一步。** 直接跑第三节的 `python3 导出照片.py`，向导发现没有模拟器时
> 会问你要不要自动装一个免费的。想用 MuMu 的话，就按下面路线 A 先装好再跑向导。
...
> **这一步也可以交给向导。** 直接跑第三节的 `python3 导出照片.py`：
> 它发现模拟器里没装 App 时会停下来，你把 apk **拖到终端窗口里**，它就替你装好；
> 没登录时也会停下来等你，登录完按回车继续。下面是手动做法。

1. 在模拟器里装上**鑫时光集家长版**：
   - **走路线 A（MuMu）**：用 MuMu 自带的**应用宝**搜「鑫时光集家长版」安装，最省事。
     （已核实：应用宝里有这个 App，分类是「启蒙早教」。）
   - **走路线 B（免费模拟器）**：它没有应用市场，需要 apk 安装包，
     把 apk **直接拖进模拟器窗口**即可安装。

   > apk 从哪来？最省事的办法是**找个已经有这个 apk 的人直接发给你**。
   > **不要从第三方 apk 下载站拿。** 你要在这个 App 里输入真实账号密码，
   > 而被二次打包的安装包会照常显示登录界面、也能登录成功，同时把你的账号发给别人，
   > 用起来完全看不出异常。
   >
   > 向导在安装前会自动校验：不是 apk 会当场拦下；apk 的校验码（SHA-256）与已记录版本
   > 对不上会警告并让你确认。已记录的版本：
   > `1.9.17` = `9394e2d829fcd2c29f01b390883e28dab82098216af4029c14f7ad8eaf2aae85`
   >
   > 校验码相同只说明**和这份记录的文件一字不差**，不等于「官方正版」；
   > App 出新版本时校验码本来就会变，那时警告属于正常，需要你自己判断来源是否可信。
...
向导会自己完成剩下的事：检查模拟器、等你登录、让你挑孩子、导出、打开文件夹。
全程只需要按几次回车、输一个数字、起个名字。

跑起来大概是这样：

```
鑫时光集 · 照片导出向导
跟着提示走就行，随时可以按 Control-C 停止。

还没检测到登录信息。请在模拟器里：
  1. 打开「鑫时光集家长版」
  2. 用你自己的账号登录
  3. 进一次照片/成长记录页面
好了之后按回车重试（输 q 退出）：        ← 不会退出，原地等你

这个账号下有 2 个孩子的档案：
  1) 最近更新 2026-04-30 · 近期 4 张照片 · “今天小朋友们一起做手工”
  2) 最近更新 2026-06-12 · 近期 9 张照片 · “毕业典礼彩排”
要导出哪个？（1-2，a=全部）：1        ← 输数字就行，不用管什么 ID

给这份导出起个名字（会存到 ~/Desktop/鑫时光集导出/名字）：小明

开始导出到 ~/Desktop/鑫时光集导出/小明 …
第 1 轮：1426/1426
全部完成，共 1 份。
已为你打开：~/Desktop/鑫时光集导出/小明
```

几个要点：

- **App 没装、没登录，向导都不会让你重来。** 模拟器里还没有这个 App 时，
  把 apk 拖到终端窗口里，向导替你装；没登录时它会停着等你，登录好回来按回车继续。
- **不用认那串 ID。** 有多个孩子时，向导会显示每个档案最近的更新日期和一句话，你一眼就能认出谁是谁。
- **每个孩子各自一个文件夹**，可以直接整个发给对应的家长。
- **再跑一次是安全的**——已下载的自动跳过，只补缺的。中途 `Control-C` 停掉也不要紧，重跑会接着下。
- **模拟器可以提前关掉。** 看到「正在读取档案信息…」之后，登录信息就已经读完了，之后全是你电脑直接连服务器下载，关掉模拟器反而更快。

<details>
<summary>不想用向导？手动敲命令（点开看）</summary>

向导只是把下面这些命令串起来，你也可以直接用：

```bash
# 导出账号下全部孩子（混在一起，存到 build/）
python3 tools/export_originals.py api --yes

# 先看看账号下有几个孩子（会打印档案 ID）
python3 tools/export_originals.py api --list-children

# 只导出某一个孩子，存到指定目录
python3 tools/export_originals.py api --child-id <档案ID> --out ~/Desktop/导出/小明 --yes
```

档案 ID 属于个人信息，别截图外发。完整选项见「五、常用选项」。
...
| 提示 | 原因 / 解决 |
|---|---|
| （用向导时不会看到这些代码）| `python3 导出照片.py` 会把常见问题翻译成人话并原地等你处理，下面这些代码只在手动敲命令时出现。 |
...
- 运行测试：`python3 -m unittest discover -s tests -t .`（278 项，全部离线，不碰真实设备或网络）
...
- `导出照片.py` 是根目录入口，逻辑在 `tools/wizard.py`：一问一答地串起设备检查、登录等待、档案选择、导出、打开文件夹。向导不重复实现任何采集逻辑，最终仍调用 `feed_api.run_api`，所以两条路行为一致。交互函数一律注入 `input_fn` / `printer`，测试因此可以全离线跑。
