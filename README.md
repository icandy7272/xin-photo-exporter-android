...
| **安卓模拟器** | 见步骤 3。首选 **MuMu**（Mac 版有 7 天免费试用，一次性导出够用）；试用到期或不想装，用工具自带的免费方案兜底。 |
| **鑫时光集家长版** App | 装进模拟器并登录，见步骤 4。 |
| **Python 3** | macOS 一般自带；下面教你确认。**不需要安装任何额外的库。** |
| 硬盘空间 | 相册本身几百 MB 到几 GB；走免费模拟器的话另加约 6 GB（一次性）。 |
...
第一次运行要下载约 4.5 GB（Java、安卓工具、系统镜像），大概十几到二十几分钟。
中途会问你几次**是否接受 Android 的许可协议**，看到 `Accept? (y/N):` 输入 `y` 回车即可。
装好后会自动弹出一个安卓手机窗口，界面是中文的。

> **不需要 Homebrew，也不需要管理员密码。** 所有东西装在 `~/.xin-exporter/` 里，
> 删掉这个文件夹就等于卸载干净（模拟器另存在 `~/.android/avd/`，用本节末尾的命令清理）。

两条路工具都支持，会自动认出当前连着的是哪一种，命令完全一样。

<details>
<summary>已经有安卓设备？（点开看）</summary>

- **已经装了 Android Studio**：路线 B 的脚本会自动复用它的 SDK 和 Java，不重复下载。
- **已经 root 过的安卓手机**：手机上开启「开发者选项 → USB 调试」，数据线连上 Mac 就行，
  两条路都不用走。工具会自动通过 `su` 读取。**没有 root 的手机不行**——系统不允许读 App 的私有数据。
...
1. 在模拟器里装上**鑫时光集家长版**：
   - **走路线 A（MuMu）**：用 MuMu 自带的**应用市场**搜「鑫时光集家长版」安装，最省事。
   - **走路线 B（免费模拟器）**：它没有应用市场，需要 apk 安装包，
     把 apk **直接拖进模拟器窗口**即可安装。
...
- **走路线 A（MuMu）**：在模拟器里打开鑫时光集，**退出登录**；不打算再用就直接卸载 MuMu。
- **走路线 B（免费模拟器）**：一条命令删掉模拟器，登录状态随之清除：

  ```bash
  python3 tools/setup_emulator.py --delete
  ```

  以后要再导，重跑一次 `python3 tools/setup_emulator.py` 即可——下载过的东西会跳过，几十秒就好。
  想彻底卸载：再删掉 `~/.xin-exporter/` 文件夹。

已经导出到 `build/`（或 `--out` 目录）的照片不受影响。
...
| `no-device` | 模拟器没开 → 打开 MuMu（路线 A），或重新运行 `python3 tools/setup_emulator.py`（路线 B），等它进入安卓桌面再重试。 |
| `prefs-read-failed` | 设备不肯交出 App 的私有数据，**是 Root 权限问题**（跟没登录不是一回事）→ **用 MuMu 的话，九成是没开 Root**：设置 → 其他/系统 → 打开 Root 权限，然后重启模拟器。路线 B 的模拟器不会出这个；自己手建的模拟器多半是选了 Google Play 镜像（必须选 **Google APIs**）；真机必须已 root。 |
